...
def validate(model, dataloader):
    ...
    with torch.no_grad():
        for (batch_seqs, batch_seq_masks, batch_seq_segments, batch_labels) in dataloader:
            ...
            all_prob.extend(probabilities[:,2].cpu().numpy())
            all_p_label.extend(probabilities.cpu().numpy())
            all_labels.extend(batch_labels)
    epoch_time = time.time() - epoch_start
    epoch_loss = running_loss / len(dataloader)
    epoch_accuracy = running_accuracy / (len(dataloader.dataset))
    return epoch_time, epoch_loss, epoch_accuracy, roc_auc_score(all_labels, all_p_label, multi_class="ovo"), all_prob
...
def test(model, dataloader):
    ...
    Returns:
        ...
        all_prob: The probability of classification on the entire validation set.
    """
