        ...
        if epoch_accuracy < best_score:
            ...
            if (if_save_model):
                  torch.save({"epoch": epoch, 
                           ...
                           os.path.join(target_dir, "best.pth.bin"))
            ...
            test_prediction = pd.DataFrame({'prob':all_prob})
            predict_class = []
            for i in all_prob:
                predict_class.append(pd.Series(i).argmax())
            test_prediction['prediction'] = predict_class
            test_prediction = test_prediction[['prob', 'prediction']]
...
def model_load_test(test_df,target_dir, max_seq_len=50, batch_size=32):
    """
    Parameters
    ----------
    test_df : pandas dataframe of test set.
    ...
    if platform == "linux" or platform == "linux2":
        checkpoint = torch.load(os.path.join(target_dir, "best.pth.bin"))
    else:
        checkpoint = torch.load(os.path.join(target_dir, "best.pth.bin"), map_location=device)
    ...
    class_map = {0:"negtive", 1:"neutral", 2:"positive"}

    return [class_map[i.argmax()] for i in all_prob]
...
if __name__ == "__main__":
    text = {"text":["this cheese cake is gross","i am fine","hello?"]}
    print(model_load_test(text,"./output", 200, 1))